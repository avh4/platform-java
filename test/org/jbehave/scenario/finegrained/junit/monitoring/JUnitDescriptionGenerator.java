...
public class JUnitDescriptionGenerator {
    ...
    public Description createDescriptionFrom(ScenarioDefinition scenario, Steps... candidateSteps) {
        Description scenarioDescription = Description.createTestDescription(candidateSteps[0]
                .getClass(), scenario.getTitle());
            ...
            for (Steps candidates : candidateSteps) {
                for (CandidateStep candidate : candidates.getSteps()) {
                    if (candidate.matches(stringStep)) {
                        String uniqueString = uniquefier
                                .getUniqueDescription(getJunitSafeString(stringStep));
                        scenarioDescription.addChild(Description.createTestDescription(candidates
                                .getClass(), uniqueString + " - Scenario: " + scenario.getTitle()
                                + ""));
                    }
                }
            }
    ...
    public Description createDescriptionFrom(StoryDefinition story, Steps candidateSteps,
